...
from models.models import Bed, Nurse, Bed_History, Patient
...
def all_patients(request):
    ...
    try:
        if request.method == "POST":
            data = request.POST.get('data')
            for i in Patient.objects.filter(Q(name__contains=data) | Q(mobile__contains=data) | Q(age__contains=data)):
...
def bed_allotment(request):
    ...
    try:
        if request.method == "POST":
            data = request.POST.get('data')
            for i in Bed.objects.filter(Q(number__contains=data) | Q(nurse__name__contains=data) | Q(Patient__name__contains=data)):
            ...
            for i in Bed.objects.all():
                try:
                    per = Patient.objects.get(bed=i.id)
    ...
    if value == 'beds':
        ...
        try:
            for i in Bed_History.objects.filter(bed=id):
                bedhistory.extend([(i.id, i.bed, i.date, i.Patient, i.sickness, i.nurse)])
...
class BedUpdate(LoginRequiredMixin, UpdateView):
    """Docstring."""
...
class BedHistoryUpdate(LoginRequiredMixin, UpdateView):
    """Docstring."""
...
class NurseUpdate(LoginRequiredMixin, UpdateView):
    """Docstring."""
